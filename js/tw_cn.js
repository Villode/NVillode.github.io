document.addEventListener('DOMContentLoaded', function () {
  const { defaultEncoding, translateDelay, msgToTraditionalChinese, msgToSimplifiedChinese, rightMenuMsgToTraditionalChinese, rightMenuMsgToSimplifiedChinese } = GLOBAL_CONFIG.translate
  ...
  function translateBody(fobj) {
    ...
    for (let i = 0; i < objs.length; i++) {
      ...
      if (
        ...
        obj === translateButtonObject || obj === translateRightMenuButtonObject
      ) {
        continue
      }
  ...
  function translatePage() {
    if (targetEncoding === 1) {
      ...
      translateRightMenuButtonObject.textContent = rightMenuMsgToTraditionalChinese
      isSnackbar && anzhiyu.snackbarShow(snackbarData.cht_to_chs)
    } else if (targetEncoding === 2) {
      currentEncoding = 2
      targetEncoding = 1
      translateButtonObject.textContent = msgToSimplifiedChinese
      translateRightMenuButtonObject.textContent = rightMenuMsgToSimplifiedChinese
  ...
  function translateInitialization() {
    ...
    translateRightMenuButtonObject = document.querySelector('#menu-translate span');
    ...
    if (translateButtonObject || translateRightMenuButtonObject) {
      if (currentEncoding !== targetEncoding) {
        const textContent = targetEncoding === 1 ? msgToSimplifiedChinese : msgToTraditionalChinese,
          RightMenu_textContent = targetEncoding === 1 ? rightMenuMsgToSimplifiedChinese : rightMenuMsgToTraditionalChinese;
        ...
        if (translateRightMenuButtonObject) {
          translateRightMenuButtonObject.textContent = RightMenu_textContent;
        }
